...
public class OperatingHoursToggle : MonoBehaviour
{
    ...
    {
        config = new();
        openHour = int.Parse(config.GetValue("OOW", "OPEN_HOUR"));
        openMinute = int.Parse(config.GetValue("OOW", "OPEN_MINUTE"));
        closeHour = int.Parse(config.GetValue("OOW", "CLOSE_HOUR"));
        closeMinute = int.Parse(config.GetValue("OOW", "CLOSE_MINUTE"));
